<script lang="ts">
  ...
  import { lottoRoundApi } from "$lib";
    ...
    onMount(async () => {
      ...
      console.log('lottoRoundData', lottoRoundData);
        ...
        <div class="p-2">
          <div class="flex flex-col sm:flex-row justify-between border-b pb-4">
            <div class="flex flex-wrap items-center mb-2 sm:mb-0">
              ...
              <p class="text-xs sm:text-sm blink">เดิมพันก่อนภายในเวลา {formattedTime}</p>
            </div>
          ...
          <div class="flex border-l relative justify-start mt-4">
            <button
              class="flex items-center justify-center text-center cursor-pointer py-2 px-3 sm:px-4 text-xs sm:text-sm border-r border-t relative border-b {isCustomPlay ? 'border-b-white' : ''}"
              on:click={() => togglePlayModeWrapper("custom")}
            >
              <div
                class="absolute bg-teal-600 h-1 -top-1 {isCustomPlay ? 'block' : 'hidden'}"
