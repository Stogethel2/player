<script lang="ts">
    ...
    onMount(async () => {
        ...
        playlistTH = lottoData.filter(
            (data) => data.lotto_category == "หวยไทย"
        );
...
{#each uniqueLotteries as lottery}
    {#if lottery.lotto_category}
        <div
            class="grid border-8 border-gray-200 rounded-[1rem] shadow-md bg-white p-4 mb-8"
        >
            <div
                class="flex flex-row items-center border-b border-gray-300 pb-4"
            >
