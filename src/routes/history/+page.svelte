...
{:else}
  <div class="max-w-4xl mx-auto p-4">
    ...
    <div class="space-y-4">
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          >
            ...
            <div class="flex items-center space-x-3">
              <span
                class={`px-2 py-1 text-xs rounded-full ${getStatusColor(order.status)}`}
              >
                {order.status}
              </span>
              <ChevronDown
                class={`w-5 h-5 text-gray-400 transition-transform ${
                  selectedOrderId === order.id ? "transform rotate-180" : ""
                }`}
              />
            </div>
          </button>
              ...
              <div class="space-y-3">
                {#each order.orderBets as bet}
                  ...
                  >
                    ...
                    <div>
                      <p class="text-gray-600">จำนวนเงิน</p>
                      <p class="font-medium">฿{bet.bet_amount.toFixed(2)}</p>
                    </div>
                    <div>
      ...
      {#if orders.length === 0}
        <div class="text-center py-8 text-gray-500">ไม่พบประวัติการแทง</div>
      {/if}
    </div>
  </div>
{/if}
